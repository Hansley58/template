...
function revealContent() {
 ...
 const image = document.getElementById('rotatingImage');
  ...
  // Function to check if #folio is in view
  function checkFolioInView() {
    const folio = document.getElementById('folio');
    const rect = folio.getBoundingClientRect();
    
    if (rect.top <= window.innerHeight && rect.bottom >= 0) {
      folio.classList.add('visible');
    } else {
  ...
  window.addEventListener('scroll', checkFolioInView);
  
  // Run check on page load to handle pre-scrolled states
  checkFolioInView();
